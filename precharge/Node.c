...
NodeType Node_Diag_False = { 0, Diagnosis, 2, 0, PrechargeM_StartPre,1 };	//无故障 开始预充 进入预充完成判断Node1
//Node1 预充完成判断
NodeType Node_Prechrge_True = { 1, PrechargeM_IsFinish, 2, , RelayM_Change, 2 };	//预充完成， 断开预充，闭合总正 进入下一节点Node2
NodeType Node_Prechrge_False = { 1, PrechargeM_IsFinish, 2, 0, No_Act, 3 };		//预充未完成 进入超时判断Node3
//Node2	充电中故障判断
NodeType Node_Charging_TRUE = { 2, Diagnosis, 2, 1, PrechargeM_StopMaster, 0 };//故障 断开总正 进入Node0
NodeType Node_Charging_False = { 2, Diagnosis, 2, 0, No_Act, 2 };		//充电中 继续充电
//Node3 超时判断
NodeType Node_Fail_True = { 3, PrechargeM_IsFail, 2, 1,PrechargeM_StopPre, 0 };	//超时 预充停止 进入下一节点Node0
